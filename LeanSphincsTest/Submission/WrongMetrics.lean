...
/-! Comparator canary fixture `WrongMetrics`. Metric statements only: no signature
...
theorem LeanSphincsTest.candidate :
    LeanSphincsTest.MetricClaim LeanSphincsTest.Submission.scheme 1 2 [⟨256, 0, 2, 0, 256⟩] := by
  refine ⟨?_, ?_, ?_⟩
