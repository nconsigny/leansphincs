#!/usr/bin/env bash
# Adapted from proximity-prize (Apache-2.0); changed challenge names, metric
# files, import prefixes and native_decide policy. See THIRD_PARTY_NOTICES.md.
set -euo pipefail

if [[ "$#" -ne 1 ]]; then
  echo "usage: check-submission-imports.sh SUBMISSION_DIR" >&2
  exit 2
fi
submission_dir="$1"
submission_module="LeanSphincs.Submission."
target_module="LeanSphincs.Benchmark.Target"
...
for required in Scheme.lean Solution.lean sigma.txt hverify.txt bound.txt; do
